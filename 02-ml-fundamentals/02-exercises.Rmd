...
# Or take the median!
naive_guess2 <- median(dat$FE)

# Perhaps the mean fuel economy from the last year?
naive_guess3 <- mean(cars2012$FE)

# Or even the median fuel economy from the last year
naive_guess4 <- median(cars2012$FE)
```
...
samp %>%
  ggplot(aes(x=EngDispl, y = FE))  +
  geom_point() +
  geom_hline(yintercept = naive_guess, color = "red") +
...
```{r}
fit.2010 <- lm(formula = FE ~ EngDispl,
               data = cars2010)
```
...
rmse.2010 <- rmse(cars2010$FE,
                  y.2010)
rmse.2011 <- rmse(cars2011$FE,
                  y.2011)
rmse.2012 <- rmse(cars2012$FE,
                  y.2012)
...
```{r}
# Note 1: Corrected object that was fit.2010 to be fit.2011 so as not
# to overwrite fit.2010 object above
# Note 2: No variable named "VarValve" in cars2011, used both variables
# that have VarValve* in them
fit.2011 <- lm(formula = FE ~ EngDispl + NumCyl + VarValveTiming + VarValveLift,
               data = cars2011)
betas <- fit.2011$coefficients
```